...
- **Api / Cli** — thin composition roots. The API exposes `/health` (liveness) and `/diagnostics` (both minimal-API), plus the Phase 5 `POST /query` **MVC controller** (`Controllers/QueryController` — the API's first controller; Swagger UI at `/swagger` in development, `AddControllers`/`MapControllers` alongside the existing minimal APIs). The CLI exposes `doctor` and the Phase 5 `ask` command (one-shot when a question is passed, otherwise an interactive REPL that keeps follow-up history in-process and supports `:save`/`:quit`). Diagnostics run the **same three checks** via `IDiagnosticsService`.

### Stub convention

Adapters not yet needed (`OracleProjectRepository`) are registered in DI but **throw `NotImplementedException` with a "...not implemented until Phase N" message** so accidental use fails loudly. When implementing a later phase, fill the matching stub — don't add a parallel type. Real today: the embedding/chat/Oracle-health diagnostics paths, the file-backed wiki store/repository (Phase 1), the ingestion service (Phase 2), the Oracle VECTOR + Oracle Text hybrid store (`OracleVectorStore`, Phase 4), and the query/synthesis service (`QueryService`, Phase 5). Oracle project persistence remains a stub until Phase 6.
